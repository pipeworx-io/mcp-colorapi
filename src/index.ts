...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'Color API');
}
...
async function identifyColor(hex: string) {
  ...
  const res = await pwFetch(`${BASE_URL}/id?hex=${encodeURIComponent(clean)}`);
  if (!res.ok) throw await httpError(res, 'Color API error');
...
async function generateScheme(hex: string, mode: string, count: number) {
  ...
  const res = await pwFetch(`${BASE_URL}/scheme?${params}`);
  if (!res.ok) throw await httpError(res, 'Color API error');
...
async function convertColor(r: number, g: number, b: number) {
  ...
  const res = await pwFetch(`${BASE_URL}/id?${params}`);
  if (!res.ok) throw await httpError(res, 'Color API error');
  const data = (await res.json()) as RawColor;
  return formatColor(data);
}
