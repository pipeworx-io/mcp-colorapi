...
`tools/list` at `https://gateway.pipeworx.io/colorapi/mcp` returns the tools in the table
...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/identify_color \
  -H 'Content-Type: application/json' \
  -d '{"hex":"FF5733"}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/identify_color`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
